...
pub const meta = .{
    ...
    .doc = "RFC 3912 whois client — query format + referral chasing (IANA→registrar) + field extraction, transport-agnostic seam",
...
test "TcpTransport: a canceled read surfaces error.Canceled, not error.TransportFailed" {
    ...
    var tcp: TcpTransport = .{ .io = cue.io(), .deny_special_use = false };
    var buf: [256]u8 = undefined;

    var fut = try io.concurrent(exchangeOnce, .{ tcp.transport(), "127.0.0.1", port, &buf });
    // Cancel once the client is inside read #1, the reply read (`ReadCueIo`):
    // it has connected and written the query, so nothing earlier can take it.
...
test "TcpTransport: timeout_ms = null (default) preserves today's unbounded behaviour" {
    ...
    var cue: ReadCueIo = .init(io);
    var tcp: TcpTransport = .{ .io = cue.io(), .deny_special_use = false }; // timeout_ms left at default null
    var buf: [256]u8 = undefined;
    ...
    // it has connected and written the query, so nothing earlier can take it.
    const reached = ReadCueIo.awaitReads(io, 1);
    // Then the window, which IS this test's assertion: had a default deadline
    // crept in, the read would have returned `error.Timeout` within it. A
    // loaded machine only lengthens the window, so it cannot turn this red.
    io.sleep(.fromMilliseconds(200), .awake) catch {};
    const result = fut.cancel(io);
    try testing.expect(reached);
    try testing.expectError(error.Canceled, result); // still needs an OUTSIDE cancel, not a bound
}
