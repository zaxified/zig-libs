...
//! RFC 9964 Appendix A.1 — the published JOSE examples for ML-DSA, verbatim.
//!
//! Why these are here rather than a self-signed round trip: a round trip
//! proves this module agrees with itself. It cannot catch a JOSE binding that
//! is self-consistently WRONG — a non-empty FIPS-204 context string, a
//! signature framed rather than raw, a signing input assembled differently.
//! Each vector below was produced by an independent implementation, so a
//! single wrong decision anywhere in the chain makes verification fail.
//!
//! The data is also self-checking in a way hex KATs are not: these strings
//! cannot be silently corrupted in transcription, because ML-DSA verification
//! fails on a single altered character.
//!
//! Source: RFC 9964, Appendix A.1, fetched from
//! <https://www.rfc-editor.org/rfc/rfc9964.txt>. The RFC wraps these strings
//! across lines to fit its column width; they are joined here, and the
//! decoded lengths are asserted against FIPS 204 Table 2 below so a bad join
//! shows up as a length failure rather than as a mysterious bad signature.

const std = @import("std");
const testing = std.testing;
const jwt = @import("root.zig");

/// One published (public key, signed token) pair.
const Vector = struct {
    /// The `alg` of both the JWK and the JWS protected header.
    alg: []const u8,
    /// The JWK's `kid`, which the JWS header references.
    kid: []const u8,
    /// The JWK's `pub` member: base64url of the raw FIPS-204 public key.
    pub_b64: []const u8,
    /// The complete JWS compact serialization.
    jws: []const u8,
    /// Raw public key length (FIPS 204 Table 2), asserted after decoding.
    pk_len: usize,
    /// Raw signature length (FIPS 204 Table 2), asserted after decoding.
    sig_len: usize,
};
...
/// Verify one vector at the JWS level, under the parameter set `Scheme`.
fn verifyVector(comptime Scheme: type, v: Vector, gpa: std.mem.Allocator) !void {
    var it = std.mem.splitScalar(u8, v.jws, '.');
    const h = it.next().?;
    const pl = it.next().?;
    const sig_b64 = it.next().?;
    try testing.expect(it.next() == null);

    // RFC 7515 §5.2: the signing input is the two encoded parts joined by a
    // dot, byte for byte as they appear in the token.
    const signing_input = try std.mem.concat(gpa, u8, &.{ h, ".", pl });
    defer gpa.free(signing_input);

    var pk_raw: [Scheme.PublicKey.encoded_length]u8 = undefined;
    try std.base64.url_safe_no_pad.Decoder.decode(&pk_raw, v.pub_b64);
    const pk = try Scheme.PublicKey.fromBytes(pk_raw);

    var sig_raw: [Scheme.Signature.encoded_length]u8 = undefined;
    try std.base64.url_safe_no_pad.Decoder.decode(&sig_raw, sig_b64);
    const sig = try Scheme.Signature.fromBytes(sig_raw);

    // `verify` is pure ML-DSA with an EMPTY context, which is what RFC 9964
    // requires. If this module used any other context string, or framed the
    // signature, or built the signing input differently, this call fails --
    // which is the whole point of anchoring against someone else's bytes.
    try sig.verify(signing_input, pk);

    // The same token with one payload character changed must not verify.
    const forged = try gpa.dupe(u8, signing_input);
    defer gpa.free(forged);
    forged[forged.len - 1] = if (forged[forged.len - 1] == 'A') 'B' else 'A';
    try testing.expectError(error.SignatureVerificationFailed, sig.verify(forged, pk));
}

test "RFC 9964 A.1: every published ML-DSA JOSE example verifies" {
    try verifyVector(jwt.MlDsa44, vectors[0], testing.allocator);
    try verifyVector(jwt.MlDsa65, vectors[1], testing.allocator);
    try verifyVector(jwt.MlDsa87, vectors[2], testing.allocator);
}

test "RFC 9964 A.1: the published `pub` members load through this module's JWKS path" {
    // The vectors' signatures are anchored above; this anchors the other half
    // of the binding -- that an AKP JWK carrying the RFC's own `pub` and `alg`
    // lands in the matching `Key` variant.
    const want = [_]std.meta.Tag(jwt.Key){ .ml_dsa_44, .ml_dsa_65, .ml_dsa_87 };
    for (vectors, want) |v, tag| {
        var doc: std.ArrayList(u8) = .empty;
        defer doc.deinit(testing.allocator);
        try doc.appendSlice(testing.allocator, "{\"keys\":[{\"kty\":\"AKP\",\"kid\":\"");
        try doc.appendSlice(testing.allocator, v.kid);
        try doc.appendSlice(testing.allocator, "\",\"alg\":\"");
        try doc.appendSlice(testing.allocator, v.alg);
        try doc.appendSlice(testing.allocator, "\",\"pub\":\"");
        try doc.appendSlice(testing.allocator, v.pub_b64);
        try doc.appendSlice(testing.allocator, "\"}]}");

        var set = try jwt.parseJwks(testing.allocator, doc.items);
        defer set.deinit();
        try testing.expectEqual(@as(usize, 0), set.skipped.len);
        try testing.expectEqual(@as(usize, 1), set.keys.len);
        try testing.expectEqual(tag, std.meta.activeTag(set.keys[0].key));
    }
}

test "RFC 9964 A.1: these are JWS, not JWT -- the token parser refuses them" {
    // Worth pinning rather than glossing: the RFC's payload is the plain text
    // "It's a dangerous business, Frodo, going out your door.", and a JWT
    // payload must be a JSON object (RFC 7519 §3). So these vectors CANNOT go
    // through `jwt.parse`, and the tests above verify at the JWS layer on
    // purpose -- not to route around an inconvenient failure.
    for (vectors) |v| {
        try testing.expectError(error.InvalidJson, jwt.parse(testing.allocator, v.jws));
    }
}

test "RFC 9964 A.1: the published keys derive from the RFC's own all-zero seed" {
    // The RFC gives `priv` as base64url of 32 ZERO bytes -- the FIPS-204 xi
    // seed -- for all three examples. So the published `pub` is not just data
    // to verify against: it pins KEY GENERATION too. If `generateDeterministic`
    // ever disagreed with FIPS 204's ExpandA/ExpandS, every key here would come
    // out different and this test says so, in the one place where an outside
    // authority published both halves of the pair.
    const zero_seed: [32]u8 = @splat(0);
    inline for (.{
        .{ jwt.MlDsa44, 0 },
        .{ jwt.MlDsa65, 1 },
        .{ jwt.MlDsa87, 2 },
        ...
        const Scheme = pair[0];
        const v = vectors[pair[1]];
        const kp = try Scheme.KeyPair.generateDeterministic(zero_seed);
        var want: [Scheme.PublicKey.encoded_length]u8 = undefined;
        try std.base64.url_safe_no_pad.Decoder.decode(&want, v.pub_b64);
        try testing.expectEqualSlices(u8, &want, &kp.public_key.toBytes());
    }
}

test "RFC 9964 A.1: a JWT signed with the RFC's key verifies through the module's own path" {
    // The vectors themselves are JWS over text, so they cannot exercise
    // `verify`/`verifyWithJwks`. Because the RFC publishes the SEED, we can
    // rebuild its keypair, sign a real JWT claim set with it, and run the
    // whole module path -- header alg, AKP JWK, key selection, signature
    // check -- against a key an outside authority defined rather than one
    // this test invented.
    const kp = try jwt.MlDsa65.KeyPair.generateDeterministic(@splat(0));
    const v = vectors[1]; // ML-DSA-65

    var doc: std.ArrayList(u8) = .empty;
    defer doc.deinit(testing.allocator);
    try doc.appendSlice(testing.allocator, "{\"keys\":[{\"kty\":\"AKP\",\"kid\":\"");
    try doc.appendSlice(testing.allocator, v.kid);
    try doc.appendSlice(testing.allocator, "\",\"alg\":\"ML-DSA-65\",\"pub\":\"");
    try doc.appendSlice(testing.allocator, v.pub_b64);
    try doc.appendSlice(testing.allocator, "\"}]}");
    var set = try jwt.parseJwks(testing.allocator, doc.items);
    defer set.deinit();
    ...
    var w: usize = 0;
    w += enc.encode(buf[w..], "{\"alg\":\"ML-DSA-65\",\"kid\":\"" ++ vectors[1].kid ++ "\"}").len;
    buf[w] = '.';
    w += 1;
    w += enc.encode(buf[w..], "{\"exp\":99999999999,\"sub\":\"pq-user\"}").len;
    const signing_input = buf[0..w];

    const sig = try kp.sign(signing_input, null);
    buf[w] = '.';
    const n = enc.encode(buf[w + 1 ..], &sig.toBytes()).len;
    const token = buf[0 .. w + 1 + n];

    var parsed = try jwt.parse(testing.allocator, token);
    defer parsed.deinit();
    try testing.expectEqual(jwt.Alg.@"ML-DSA-65", parsed.alg);
    try jwt.verifyWithJwks(&parsed, set);
}

test "RFC 9964 A.1: the published key and signature lengths are FIPS 204's" {
    var buf: [8192]u8 = undefined;
    for (vectors) |v| {
        const n = try std.base64.url_safe_no_pad.Decoder.calcSizeForSlice(v.pub_b64);
        try testing.expectEqual(v.pk_len, n);
        try std.base64.url_safe_no_pad.Decoder.decode(buf[0..n], v.pub_b64);

        var it = std.mem.splitScalar(u8, v.jws, '.');
        _ = it.next();
        _ = it.next();
        const sig_b64 = it.next().?;
        try testing.expectEqual(v.sig_len, try std.base64.url_safe_no_pad.Decoder.calcSizeForSlice(sig_b64));
    }
}
