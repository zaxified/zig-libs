...
pub const State = struct {
    ...
    /// `symbolCosts`: the entropy tables the block starts from (the
    /// previous block's, or a dictionary's), set for every block. A first
    /// block seeds its statistics from them when the Huffman table came
    /// from a dictionary (`HUF_repeat_valid`).
    symbol_costs: ?SymbolCosts = null,
    price_table: [opt_size]Optimal = undefined,
    match_table: [opt_size]Match = undefined,
};

/// The previous block state's entropy tables (`ZSTD_entropyCTables_t`).
pub const SymbolCosts = struct {
    huf: *const literals.HufState,
    fse: *const sequences.FseTables,
};
...
/// `FSE_getMaxNbBits` of `symbol` in `ct`, as a frequency on `scale_log`
/// bits (1 for a symbol that costs nothing).
fn freqFromFse(ct: *const @import("fse.zig").CTable, symbol: usize) u32 {
    const scale_log = 10; // scale to 1K
    const bit_cost = (ct.symbol_tt[symbol].delta_nb_bits +% ((1 << 16) - 1)) >> 16;
    std.debug.assert(bit_cost < scale_log);
    return if (bit_cost != 0) @as(u32, 1) << @intCast(scale_log - bit_cost) else 1; // minimum to calculate cost
}

/// `ZSTD_rescaleFreqs`: on the first block, statistics from the
/// dictionary's tables when it has a full Huffman table
/// (`HUF_repeat_valid`), else from the block's literals and baselines; on
/// later blocks, the previous statistics scaled down.
fn rescaleFreqs(st: *State, src: []const u8, comptime opt_level: u32) void {
    ...
    if (st.lit_length_sum == 0) { // no statistics yet: first block
        ...
        const sc = st.symbol_costs.?;
        if (sc.huf.repeat == .valid) {
            ...
            if (st.compressed_literals) {
                ...
                st.lit_sum = 0;
                for (&st.lit_freq, 0..) |*f, lit| {
                    const scale_log = 11; // scale to 2K
                    // HUF_getNbBitsFromCTable
                    const bit_cost: u32 = if (lit > sc.huf.table.max_symbol) 0 else sc.huf.table.elt[lit].nb_bits;
                    std.debug.assert(bit_cost <= scale_log);
                    f.* = if (bit_cost != 0) @as(u32, 1) << @intCast(scale_log - bit_cost) else 1; // minimum to calculate cost
                    st.lit_sum += f.*;
                }
            }
            st.lit_length_sum = 0;
            for (&st.lit_length_freq, 0..) |*f, ll| {
                f.* = freqFromFse(&sc.fse.ll, ll);
                st.lit_length_sum += f.*;
            }
            st.match_length_sum = 0;
            for (&st.match_length_freq, 0..) |*f, ml| {
                f.* = freqFromFse(&sc.fse.ml, ml);
                st.match_length_sum += f.*;
            }
            st.off_code_sum = 0;
            for (&st.off_code_freq, 0..) |*f, of| {
                f.* = freqFromFse(&sc.fse.of, of);
                st.off_code_sum += f.*;
            }
...
inline fn lowestMatchIndex(ms: *const MatchState, curr: u32) u32 {
    return ms.lowestMatchIndex(curr);
}
...
/// `ZSTD_updateTree` as dictionary loading calls it: every position from
/// `next_to_update` up to `ip` sorted into the binary tree (btlazy2 too),
/// hashed with `minMatch` as it is.
pub fn updateTreeDictionary(ms: *MatchState, ip: u32, iend: usize) void {
    switch (ms.cp.min_match) {
        inline 5, 6, 7 => |m| updateTree(ms, ip, iend, m, false),
        else => updateTree(ms, ip, iend, 4, false),
    }
}
