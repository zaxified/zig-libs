...
|---|---|---|
| `gen-goldens.sh` + `dump_corpus.zig` | **recipe** for committed goldens | Writes `src/testdata/goldens.zig`: for every case in `src/testdata/corpus.zig`, level and checksum setting, the length and SHA-256 of the frame libzstd emits; `src/testdata/stream_goldens.zig`, the same for every `corpus.stream_cases` schedule through `zstream`; `src/testdata/param_goldens.zig`, for every `corpus.param_cases` entry with its advanced parameters through `zref`; `src/testdata/cdict_goldens.zig`, for every `corpus.dict_cases` entry through `zref` or `zstream` with its dictionary (both decode each frame back with it; exit 8 if that fails); `src/testdata/mt_goldens.zig`, for every `corpus.mt_cases` entry with `nbWorkers` = `corpus.mt_golden_workers`, refusing if 1 worker gives other bytes; and `src/testdata/seq_goldens.zig`, for every `corpus.seq_cases` entry through `zseq` (the sequences it reads are `testdata/seqgen.zig`'s, which `dump_corpus.zig` writes as `seq-<case>.seq`), the output's length and SHA-256 or libzstd's error. `zref` and `zstream` are built with `ZSTD_MULTITHREAD` from the library sources (the static library is single-threaded by default); with `nbWorkers` 0 they are the single-threaded library. |
...
| `zseq.c` | **differential oracle** (sequence-level API) | Runs libzstd's `ZSTD_generateSequences`, `ZSTD_mergeBlockDelimiters`, `ZSTD_compressSequences`, `ZSTD_compressSequencesAndLiterals`, or `ZSTD_compress2` / `ZSTD_compressStream2` with an example sequence producer registered (`seq_test.zig`'s `ExampleProducer` is the same function, mode bits included), on any file, with a sequence file (16 bytes per `ZSTD_Sequence`, little-endian), `zref`'s `name=value` parameters plus the four sequence parameters, a destination capacity and a dictionary; writes the output or prints `ERR <ZSTD_getErrorName>`. Built like `zref` (multithreaded, for `nbWorkers`). |
...
| `crafted-frames.py` | **recipe** for committed goldens | Writes the frames no encoder writes that `decoder_test.zig` ("frames no encoder writes ...") builds itself byte by byte -- 4 Huffman streams of exactly 6 literals (X1, and X2 through a treeless block), RLE tables of the largest literal- and match-length codes, 0x7EFF / 0x7F00 / 0x7F01 sequences -- and prints each frame's FNV-1a, which the test's builder must match; `zdec` (modes 0, 1, 2) over them gives libzstd's output the test pins. |
...
| `zfinal.c` | **differential oracle** (finished dictionaries) | Runs libzstd's public dictBuilder API on a sample set (`ztrain.c`'s format): `ZDICT_finalizeDictionary`, `ZDICT_addEntropyTablesFromBuffer`, `ZDICT_trainFromBuffer_cover` / `_fastCover`, the two optimizers (single-threaded), `ZDICT_trainFromBuffer`, and `COVER_selectDict` (the optimizers' scorer, with shrinking, which no public trainer reaches); writes the dictionary and prints `OK <size> <header size> <a> <b>` (the optimizers' k and d, or the selection's total) or `ERR <ZSTD_ErrorCode>`. `gen-dict-goldens.sh` uses it for `src/testdata/dict_final_goldens.zig` (`dict_samples.final_runs`). Links `libzstd.a`. |
...
## The reference they need
    ...
    # with nbWorkers (multithreaded), from the sources as gen-goldens.sh does:
    #   cc -O2 -DZSTD_MULTITHREAD -DZSTD_DISABLE_ASM -pthread -I "$R/lib" -I "$R/lib/common" \
    #       -o "$R/zref" modules/zstd/tools/zref.c $R/lib/{common,compress,decompress}/*.c
...
and the finished-dictionary oracle against the static library:

    make -C "$R/lib" libzstd.a
    cc -O2 -I "$R/lib" -I "$R/lib/dictBuilder" -o "$R/zfinal" modules/zstd/tools/zfinal.c "$R/lib/libzstd.a" -lpthread
    "$R/zfinal" default samples.bin dict.bin 16384      # ZDICT_trainFromBuffer
...
## Regenerating the goldens
    ...
    ZSTD_REF=<dir> modules/zstd/tools/gen-dict-goldens.sh   # training: dict_goldens.zig, dict_final_goldens.zig
