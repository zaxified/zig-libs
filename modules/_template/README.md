...
10a. **What you build to CHECK this module** (`CONVENTIONS.md` §9). Tests,
   fuzz and ctgrind harnesses live in `src/`. Of everything else — including an
   audit's scratch — only two kinds are kept, in `modules/<name>/tools/`: the
   **recipe** that produced committed goldens/KATs/fixtures, and a
   **differential oracle** that drives a foreign implementation through the
   public API or wire format. Give each a header saying what it needs and what it
   produces. Mutation runners, probes for one finding and module snapshots are
   deleted once a test in `src/` pins the result — they anchor on source text
   and rot within days.
