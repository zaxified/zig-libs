...
pub const meta = .{
    ...
    .doc = "OCSP-stapling fetch + cache over `ocsp` — AIA responder discovery, verify-before-cache, refresh-ahead expiry, soft-fail on outage.",
...
test "httpFetch: a canceled connect/head wait surfaces error.Canceled, not error.TransportFailed" {
    ...
    var client = http.Client.init(cue.io(), testing.allocator, .{ .pool = .{ .enabled = false } });
    defer client.deinit();
    const transport = httpTransport(&client);
    ...
    // Cancel once the client is inside read #1, the head read (`ReadCueIo`):
    // it has connected and written, so nothing earlier can take the cancel.
