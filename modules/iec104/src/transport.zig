...
/// `error.EndOfStream` — and not a cancellation either, which is
/// `error.Canceled`. Conflating the three is what turns a canceled shutdown
/// into a caller that polls forever. A caller that wants a read timeout
/// implements it inside its own `read`; there is no timer thread here.
...
test "a cancel during the read timeout's poll is not reported as an idle round" {
    ...
    try fixture.tt.setReadTimeout(600);
    var buf: [apci.max_apdu_len]u8 = undefined;
    var fut = try io.concurrent(readOnce, .{ &fixture.tt, &buf });
