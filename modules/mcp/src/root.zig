...
//! handshake/tools over a generic reader/writer (stdio built in).
//!
//! The split this module enforces: **server = transport, app = tools**. The
//! server owns the protocol (JSON-RPC framing, `initialize` version
//! negotiation, `tools/list`/`tools/call` dispatch, progress notifications);
//! the application registers tools — each a name + JSON-Schema text + a
//! handler that threads **app state via a `ctx` pointer**. That ctx threading
//! is the point of this module versus thin MCP libs: a tool call is a plain
...
//!
//! Resources and prompts are **extension points**: the seed server never
//! needed them, so they are not implemented — a client asking for
//! `resources/list` etc. gets -32601 Method not found, which the spec allows
//! for capabilities the server does not advertise.
//!
...
/// JSON-RPC 2.0 standard error codes (the only codes this server emits).
...
test "jsonrpc: unknown method -> -32601; id-less unknown -> dropped" {
    var s = testServer(null);
    defer s.deinit();
    try expectResponse(&s, "{\"jsonrpc\":\"2.0\",\"id\":9,\"method\":\"resources/list\"}",
