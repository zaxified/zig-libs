...
sections; `writeText` holds the lock only to snapshot the family/children pointers
(O(number of families)) and formats from that snapshot with the lock released, so
