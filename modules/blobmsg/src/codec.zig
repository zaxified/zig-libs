...
//
// Everything below is frozen from a SINGLE real run: the OpenWRT image in
// `scripts/vm/images/` (see scripts/vm/README.md) was booted, and a small
// throwaway byte-relay (never part of this repo, never committed) was
// spliced between the real `ubus` CLI and the real `ubusd` on
// /var/run/ubus/ubus.sock, using ubus's own `-s <socket>` flag to point the
// CLI at the tap instead. Every byte below is exactly what the real client
// sent or the real daemon replied — nothing here is derived from this
// module's own encoder/decoder (per the project's governing rule: never
// derive a golden from your own output). The matching JSON strings are the
// real `ubus -S` CLI's own stdout for the SAME invocation, captured in the
// same run — so the decode assertions below are a genuine textual
// byte-parity check against `ubus -S`, closing the gap this module's
// SPEC.md backlog flagged ("a byte-parity check ... has not yet been
// built").
//
// Findings from this capture (see SPEC.md for the full writeup):
//  * The client-visible reply to INVOKE is exactly DATA then a completion
//    STATUS (OBJID + status code) — confirmed across four different real
//    provider objects (system.board, system.info, network.device.status,
//    network.interface.lan.status). No separate "ack STATUS (no OBJID)"
//    ever appeared before DATA on the client's own connection. `Client.
//    invoke` still handles this correctly (its loop treats zero-or-more
//    OBJID-less STATUS messages as an ack to skip before the completion),
//    but the real corpus never exercises that skip branch — only the
//    scripted mock daemon above does. Left as-is (defensive, not wrong);
//    recorded so nobody mistakes "documented" for "observed".
//  * The real `ubus` CLI performs a LOOKUP to resolve the object id before
//    every INVOKE (matching this client's own `lookupId` design) and
//    ALWAYS sends an empty UBUS_ATTR_DATA on a no-argument invoke — an
//    independent, real-client confirmation of daemon gotcha #1 (previously
//    verified only against this module's own scripted mock daemon).
//  * A LOOKUP reply's SIGNATURE table stores each method argument's
//    blobmsg type as an INT32 whose value is the literal BM.* constant
//    (verified below: "Boolean" decodes to 7 == BM.INT8, "Table" to
//    2 == BM.TABLE, "String" to 3 == BM.STRING, "Integer" to 5 == BM.INT32)
//    — independent confirmation this module's BM enum values match the
//    real daemon's.
//  * No BM.DOUBLE or BM.INT16 value was observed anywhere in this real
//    corpus (system.board, system.info, network.device.status,
//    network.interface.lan.status, plus the `system` object's own method
//    signature table): `ubus`'s blobmsg-JSON codec only ever emits
//    STRING/INT8(bool)/INT32/INT64/TABLE/ARRAY on this daemon version.
//    DOUBLE and INT16 remain covered only by the hand-derived goldens
//    above (this module's encode/decode API supports both; the real
//    daemon's JSON codepath simply never exercises them) — a documented
//    gap, not a fabricated real-double claim.
//  * Two disagreements were checked for and NOT found: this module's own
//    `encodeArgs`/`appendAttrU32`/`appendAttrString`/`appendAttr` reproduce
//    the real `ubus` CLI's INVOKE bytes byte-for-byte (see the two
//    "our encode reproduces" tests below), for both an empty-args and a
//    JSON-args call.

const captured_hello = [_]u8{
    0x00, 0x00, 0x00, 0x00, 0x8c, 0x7b, 0x14, 0x4e, 0x00, 0x00, 0x00, 0x04,
};

const captured_lookup_req = [_]u8{
    0x00, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10,
    0x02, 0x00, 0x00, 0x0b, 0x73, 0x79, 0x73, 0x74, 0x65, 0x6d, 0x00, 0x00,
};

const captured_lookup_data_system = [_]u8{
    ,
};

const captured_invoke_board_req = [_]u8{
    0x00, 0x05, 0x00, 0x02, 0xaa, 0xa5, 0x4b, 0xf5, 0x00, 0x00, 0x00, 0x1c,
    0x03, 0x00, 0x00, 0x08, 0xaa, 0xa5, 0x4b, 0xf5, 0x04, 0x00, 0x00, 0x0a,
    0x62, 0x6f, 0x61, 0x72, 0x64, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x04,
};

const captured_data_board = [_]u8{
    ,
};

const captured_status_board = [_]u8{
    0x00, 0x01, 0x00, 0x02, 0xaa, 0xa5, 0x4b, 0xf5, 0x00, 0x00, 0x00, 0x14,
    0x01, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x08,
    0xaa, 0xa5, 0x4b, 0xf5,
};

const captured_invoke_devstatus_req = [_]u8{
    0x00, 0x05, 0x00, 0x02, 0x42, 0xdd, 0x38, 0xc6, 0x00, 0x00, 0x00, 0x30,
    0x03, 0x00, 0x00, 0x08, 0x42, 0xdd, 0x38, 0xc6, 0x04, 0x00, 0x00, 0x0b,
    0x73, 0x74, 0x61, 0x74, 0x75, 0x73, 0x00, 0x00, 0x07, 0x00, 0x00, 0x18,
    0x83, 0x00, 0x00, 0x11, 0x00, 0x04, 0x6e, 0x61, 0x6d, 0x65, 0x00, 0x00,
    0x65, 0x74, 0x68, 0x30, 0x00, 0x00, 0x00, 0x00,
};

const captured_data_devstatus = [_]u8{
    ,
    ...
    ,
};

const captured_data_ifstatus = [_]u8{
    ,
};

test "real ubusd capture: HELLO greeting frame decodes (type/seq/empty top attr)" {
    const h = parseMsgHeader(captured_hello[0..msghdr_len]);
    try testing.expectEqual(@as(u8, 0), h.version);
    try testing.expectEqual(MSG.HELLO, h.type);
    try testing.expectEqual(@as(u16, 0), h.seq);
    // peer is the daemon-assigned client id for this connection — varies
    // per boot, not asserted; only the frame shape is.
    const top_len = std.mem.readInt(u32, captured_hello[msghdr_len..][0..4], .big) & LEN_MASK;
    try testing.expectEqual(@as(u32, 4), top_len); // empty top attr, header only
}

test "real ubusd capture: `ubus -s <sock> list system` LOOKUP request is byte-identical to the hand-derived golden above" {
    // This is the real `ubus` CLI's own LOOKUP(objpath=\"system\") request —
    // compare it against "golden: ubus LOOKUP message frame" above, built
    // from this module's own encoder with the same seq/peer/pattern. Any
    // future encoder change that breaks real-client compatibility would
    // diverge here even though that hand test still passes on its own.
    const gpa = testing.allocator;
    var children: std.ArrayList(u8) = .empty;
    defer children.deinit(gpa);
    try appendAttrString(gpa, &children, ATTR.OBJPATH, "system");
    const msg = try encodeMessage(gpa, MSG.LOOKUP, 1, 0, children.items);
    defer gpa.free(msg);
    try testing.expectEqualSlices(u8, &captured_lookup_req, msg);
}

test "real ubusd capture: LOOKUP DATA reply for \"system\" — SIGNATURE decodes to the real ubus -S schema, BM type codes confirmed" {
    const gpa = testing.allocator;
    const top_len = std.mem.readInt(u32, captured_lookup_data_system[msghdr_len..][0..4], .big) & LEN_MASK;
    const payload = captured_lookup_data_system[msghdr_len + 4 .. msghdr_len + top_len];
    var it: AttrIterator = .{ .buf = payload };
    var saw_signature = false;
    while (try it.next()) |a| {
        if (a.id != ATTR.SIGNATURE) continue;
        saw_signature = true;
        const json = try decodeToJsonAlloc(gpa, a.data);
        defer gpa.free(json);
        // Frozen from the SAME capture's `ubus -v list system` signature —
        // "Boolean"=7=BM.INT8, "Table"=2=BM.TABLE, "String"=3=BM.STRING,
        // "Integer"=5=BM.INT32 (see the header comment above).
        try testing.expectEqualStrings(
            \\{"board":{},"info":{},"reboot":{},"watchdog":{"frequency":5,"timeout":5,"magicclose":7,"stop":7},"signal":{"pid":5,"signum":5},"validate_firmware_image":{"path":3},"sysupgrade":{"path":3,"force":7,"backup":3,"prefix":3,"command":3,"options":2}}
        , json);
    }
    try testing.expect(saw_signature);
}

test "real ubusd capture: INVOKE(system.board) request carries the empty UBUS_ATTR_DATA gotcha — a REAL client, not our own mock" {
    const top_len = std.mem.readInt(u32, captured_invoke_board_req[msghdr_len..][0..4], .big) & LEN_MASK;
    const payload = captured_invoke_board_req[msghdr_len + 4 .. msghdr_len + top_len];
    var it: AttrIterator = .{ .buf = payload };
    var saw_empty_data = false;
    while (try it.next()) |a| {
        if (a.id == ATTR.METHOD) try testing.expectEqualStrings("board", std.mem.trimEnd(u8, a.data, "\x00"));
        if (a.id == ATTR.DATA) {
            try testing.expectEqual(@as(usize, 0), a.data.len);
            saw_empty_data = true;
        }
    }
    try testing.expect(saw_empty_data);
}

test "our encode reproduces the real ubus CLI's INVOKE bytes exactly (system.board, empty args)" {
    const gpa = testing.allocator;
    var children: std.ArrayList(u8) = .empty;
    defer children.deinit(gpa);
    try appendAttrU32(gpa, &children, ATTR.OBJID, 0xaaa54bf5); // the real objid this capture resolved
    try appendAttrString(gpa, &children, ATTR.METHOD, "board");
    try appendAttr(gpa, &children, ATTR.DATA, &.{});
    const msg = try encodeMessage(gpa, MSG.INVOKE, 2, 0xaaa54bf5, children.items);
    defer gpa.free(msg);
    try testing.expectEqualSlices(u8, &captured_invoke_board_req, msg);
}

test "real ubusd capture: INVOKE(system.board) reply decodes byte-identical to the real `ubus -S call system board` stdout" {
    const gpa = testing.allocator;
    const top_len = std.mem.readInt(u32, captured_data_board[msghdr_len..][0..4], .big) & LEN_MASK;
    const payload = captured_data_board[msghdr_len + 4 .. msghdr_len + top_len];
    var it: AttrIterator = .{ .buf = payload };
    var checked = false;
    while (try it.next()) |a| {
        if (a.id != ATTR.DATA) continue;
        const json = try decodeToJsonAlloc(gpa, a.data);
        defer gpa.free(json);
        try testing.expectEqualStrings(
            \\{"kernel":"6.12.87","hostname":"OpenWrt","system":"Intel(R) Core(TM) i7-7920HQ CPU @ 3.10GHz","model":"QEMU Ubuntu 26.04 PC (i440FX + PIIX, 1996)","board_name":"qemu-ubuntu-26-04-pc-i440fx-piix-1996","rootfs_type":"ext4","release":{"distribution":"OpenWrt","version":"25.12.4","firmware_url":"https://downloads.openwrt.org/","revision":"r32933-4ccb782af7","target":"x86/64","description":"OpenWrt 25.12.4 r32933-4ccb782af7","builddate":"1778712129"}}
        , json);
        checked = true;
    }
...
test "real ubusd capture: completion STATUS carries OBJID + status 0 — and no separate ack STATUS was ever observed (see header)" {
    const h = parseMsgHeader(captured_status_board[0..msghdr_len]);
    try testing.expectEqual(MSG.STATUS, h.type);
    const top_len = std.mem.readInt(u32, captured_status_board[msghdr_len..][0..4], .big) & LEN_MASK;
    const payload = captured_status_board[msghdr_len + 4 .. msghdr_len + top_len];
    var it: AttrIterator = .{ .buf = payload };
    var status: ?i32 = null;
    var objid: ?u32 = null;
    while (try it.next()) |a| {
        if (a.id == ATTR.STATUS) status = std.mem.readInt(i32, a.data[0..4], .big);
        if (a.id == ATTR.OBJID) objid = std.mem.readInt(u32, a.data[0..4], .big);
    }
    try testing.expectEqual(@as(?i32, 0), status);
    try testing.expectEqual(@as(?u32, 0xaaa54bf5), objid);
}

test "our encode reproduces the real ubus CLI's INVOKE bytes exactly (network.device.status, JSON args)" {
    const gpa = testing.allocator;
    var parsed = try std.json.parseFromSlice(std.json.Value, gpa, "{\"name\":\"eth0\"}", .{});
    defer parsed.deinit();
    const argdata = try encodeArgs(gpa, parsed.value);
    defer gpa.free(argdata);

    var children: std.ArrayList(u8) = .empty;
    defer children.deinit(gpa);
    try appendAttrU32(gpa, &children, ATTR.OBJID, 0x42dd38c6); // the real objid this capture resolved
    try appendAttrString(gpa, &children, ATTR.METHOD, "status");
    try appendAttr(gpa, &children, ATTR.DATA, argdata);
    const msg = try encodeMessage(gpa, MSG.INVOKE, 2, 0x42dd38c6, children.items);
    defer gpa.free(msg);
    try testing.expectEqualSlices(u8, &captured_invoke_devstatus_req, msg);
}

test "real ubusd capture: INVOKE(network.device.status) reply decodes byte-identical to real `ubus -S` (bool + array + nested-table coverage)" {
    const gpa = testing.allocator;
    const top_len = std.mem.readInt(u32, captured_data_devstatus[msghdr_len..][0..4], .big) & LEN_MASK;
    const payload = captured_data_devstatus[msghdr_len + 4 .. msghdr_len + top_len];
    var it: AttrIterator = .{ .buf = payload };
    ...
    while (try it.next()) |a| {
        ...
        defer gpa.free(json);
        try testing.expectEqualStrings(
            \\{"external":false,"present":true,"type":"Network device","up":true,"carrier":true,"auth_status":false,"link-advertising":["10baseT-H","10baseT-F","100baseT-H","100baseT-F","1000baseT-F"],"link-partner-advertising":[],"link-supported":["10baseT-H","10baseT-F","100baseT-H","100baseT-F","1000baseT-F"],"speed":"1000F","autoneg":true,"flow-control":{"autoneg":true,"supported":[],"link-advertising":[],"link-partner-advertising":[],"negotiated":[]},"hw-tc-offload":false,"devtype":"ethernet","mtu":1500,"mtu6":1500,"macaddr":"52:54:00:12:34:56","txqueuelen":1000,"ipv6":false,"ip6segmentrouting":false,"promisc":false,"rpfilter":0,"acceptlocal":false,"igmpversion":0,"mldversion":0,"neigh4reachabletime":30000,"neigh6reachabletime":30000,"neigh4gcstaletime":60,"neigh6gcstaletime":60,"neigh4locktime":100,"dadtransmits":1,"multicast":true,"sendredirects":true,"drop_v4_unicast_in_l2_multicast":false,"drop_v6_unicast_in_l2_multicast":false,"drop_gratuitous_arp":false,"drop_unsolicited_na":false,"arp_accept":false,"gro":true,"statistics":{"collisions":0,"rx_frame_errors":0,"tx_compressed":0,"multicast":0,"rx_length_errors":0,"tx_dropped":0,"rx_bytes":3712528,"rx_missed_errors":0,"tx_errors":0,"rx_compressed":0,"rx_over_errors":0,"tx_fifo_errors":0,"rx_crc_errors":0,"rx_packets":2499,"tx_heartbeat_errors":0,"rx_dropped":0,"tx_aborted_errors":0,"tx_packets":292,"rx_errors":0,"tx_bytes":18129,"tx_window_errors":0,"rx_fifo_errors":0,"tx_carrier_errors":0}}
        , json);
        checked = true;
    }
    try testing.expect(checked);
...
test "real ubusd capture: INVOKE(network.interface.lan.status) reply decodes byte-identical to real `ubus -S` (array-of-tables coverage)" {
    const gpa = testing.allocator;
    const top_len = std.mem.readInt(u32, captured_data_ifstatus[msghdr_len..][0..4], .big) & LEN_MASK;
    const payload = captured_data_ifstatus[msghdr_len + 4 .. msghdr_len + top_len];
    var it: AttrIterator = .{ .buf = payload };
    var checked = false;
    while (try it.next()) |a| {
        if (a.id != ATTR.DATA) continue;
        ...
        defer gpa.free(json);
        try testing.expectEqualStrings(
            \\{"up":true,"pending":false,"available":true,"autostart":true,"dynamic":false,"uptime":5,"l3_device":"br-lan","proto":"static","device":"br-lan","updated":["addresses"],"metric":0,"dns_metric":0,"delegation":true,"ipv4-address":[{"address":"192.168.1.1","mask":24}],"ipv6-address":[],"ipv6-prefix":[],"ipv6-prefix-assignment":[{"address":"fd32:d6f0:19fa::","mask":60,"local-address":{"address":"fd32:d6f0:19fa::1","mask":60}}],"route":[],"dns-server":[],"dns-search":[],"neighbors":[],"inactive":{"ipv4-address":[],"ipv6-address":[],"route":[],"dns-server":[],"dns-search":[],"neighbors":[]},"data":{}}
        , json);
        checked = true;
    }
    try testing.expect(checked);
...
test "real capture count canary: nine frozen messages, byte lengths unchanged" {
    // Guards against an accidental truncation/edit of the const arrays
    // above going unnoticed (they're too large to eyeball on every diff).
    try testing.expectEqual(@as(usize, 12), captured_hello.len);
    try testing.expectEqual(@as(usize, 24), captured_lookup_req.len);
    try testing.expectEqual(@as(usize, 412), captured_lookup_data_system.len);
    try testing.expectEqual(@as(usize, 36), captured_invoke_board_req.len);
    try testing.expectEqual(@as(usize, 540), captured_data_board.len);
    try testing.expectEqual(@as(usize, 28), captured_status_board.len);
    try testing.expectEqual(@as(usize, 56), captured_invoke_devstatus_req.len);
    try testing.expectEqual(@as(usize, 2000), captured_data_devstatus.len);
    try testing.expectEqual(@as(usize, 792), captured_data_ifstatus.len);
}
