...
test "protocol one: authenticate/verify, tamper and wrong-length rejection" {
    ...
    const sig_token = try One.authenticate(&token16, msg);
...
test "protocol one: an empty key is rejected, not silently authenticated (audit finding L2)" {
    const msg = "ctap2 pinUvAuthToken payload";
    try t.expectError(error.EmptyKey, One.authenticate(&[_]u8{}, msg));
    // verify must fail closed on the same input, not propagate the error
    // or (worse) treat "no key" as "any signature over this message is
    // valid" -- a fixed 16-byte value stands in for an attacker-supplied
    // signature; before the fix, `One.authenticate("")` silently produced
    // a real HMAC over the all-zero key, which `verify` then accepted.
    const some_sig: [16]u8 = @splat(0xAB);
    try t.expect(!One.verify(&[_]u8{}, msg, &some_sig));
}
...
test "protocol two: authenticate/verify, tamper and wrong-length rejection" {
    ...
    try t.expect(Two.verify(key[0..32], msg, &sig));
    ...
    for (0..Two.signature_length) |i| {
        var bad_sig = sig;
        bad_sig[i] ^= 0x01;
        try t.expect(!Two.verify(key[0..32], msg, &bad_sig));
    }
    try t.expect(!Two.verify(key[0..32], "ctap2 clientPin command payloae", &sig));
    // Wrong-length signatures fail closed (incl. protocol-one-style 16).
    try t.expect(!Two.verify(key[0..32], msg, sig[0..16]));
    try t.expect(!Two.verify(key[0..32], msg, ""));
    ...
    try t.expect(!Two.verify(key[0..32], msg, &sig_token));
}
...
test "Impl(): comptime dispatch maps the wire enum to the namespaces" {
    ...
    // The generic path works for both protocols. `authenticate`'s return
    // type and `verify`'s key type genuinely differ between the two (One:
    // fallible []const u8 key -- audit L2; Two: infallible *const [32]u8
    // key -- audit H2), so this branches on the comptime protocol instead
    ...
    inline for (.{ .one, .two }) |p| {
        ...
        if (p == .one) {
            ...
            const key = enc.shared_secret[0..32];
            const sig = P.authenticate(key, "m");
            try t.expect(P.verify(key, "m", &sig));
        }
    }
}
