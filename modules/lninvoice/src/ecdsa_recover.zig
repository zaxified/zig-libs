...
//! Recoverable ECDSA/secp256k1 over `k256` — BOLT#11's `signature` field is
//! a *recoverable* compact ECDSA signature (`r || s || recid`), letting a
//! reader recover the payee's node ID straight from the signature when no
//! `n` field is present. `k256/src/sign.zig` was checked (its own doc
//! comment + exports) and ships `bip340Sign`/`bip340Verify` (Schnorr) plus
//! `ecdsaVerify` (plain ECDSA verify) — but no ECDSA **sign** and no public
//! **key recovery**. Both are implemented here from k256's own field/group/
//! scalar primitives (`k256.Secp256k1`/`k256.Fe`/`k256.Scalar`), nothing
//! upstream needed changing:
//!
...
//!     which is what lets `bolt11.zig`'s round-trip test re-derive an
//!     official vector's signature byte-for-byte from its private key —
//!     the strongest form of "the encoder is correct" the module has.
//!   * `recoverPubkey` — standard ECDSA public-key recovery,
//!     `Q = r⁻¹·(s·R − e·G)`, `R` lifted from `r` + the recovery id's parity
//!     bit via `k256.Secp256k1.recoverY`. This is the module's actual
//!     security core: `bolt11.zig`'s vectors recover the spec's own node ID
//!     from a genuine signature, and recover a WRONG (or non-recoverable)
//!     key from a tampered one — see its "tampered invoice" tests.
//!
...
const Fe = k256.Fe;
const Scalar = k256.Scalar;
const scalar_mod = k256.scalar;
...
/// already computed (BOLT#11: `SHA256(hrp || data-without-signature)`).
pub fn sign(privkey: [32]u8, hash32: [32]u8) SignError!Signature {
    ...
    if (Ra.x.toInt() >= scalar_mod.field_order) recid |= 2;
    ...
    // to the "other" nonce n-k, whose R has the OPPOSITE y parity] -- every
    // Bitcoin-ecosystem signer (and BOLT#11's own worked examples) always
    // reports the low-S form, so the parity bit must flip in lockstep with
    // negating `s` to keep recovery self-consistent.
    var s_final = s;
...
pub fn recoverPubkey(hash32: [32]u8, r: [32]u8, s: [32]u8, recid: u2) RecoverError!Secp256k1 {
    ...
    if (recid & 2 != 0) {
        x_wide += scalar_mod.field_order;
        if (x_wide >= k256.field.field_order) return error.InvalidScalar;
    }
...
/// BOLT#11 requires when an `n` field pins the signer — high-S is only
/// tolerated on the recovery path, which accepts both per BOLT#11
/// "otherwise: MUST perform ECDSA public-key recovery and accept both
/// high-S and low-S signatures").
pub fn isLowS(s: [32]u8) bool {
    const v = std.mem.readInt(u256, &s, .big);
    return v <= (scalar_mod.field_order >> 1);
}
...
test "sign then recoverPubkey round-trips to the signer's own pubkey" {
    ...
    while (i < 50) : (i += 1) {
        ...
        // ecdsaVerify (k256's own, independent implementation) agrees too.
        var sig_rs: [64]u8 = undefined;
        sig_rs[0..32].* = sig.r;
        sig_rs[32..64].* = sig.s;
        try testing.expect(k256.sign.ecdsaVerify(&want_pub.toCompressedSec1(), &msg, sig_rs));
    }
}
...
test "isLowS: half-order boundary" {
    ...
    std.mem.writeInt(u256, &half, scalar_mod.field_order >> 1, .big);
    try testing.expect(isLowS(half));
    var high: [32]u8 = undefined;
    std.mem.writeInt(u256, &high, (scalar_mod.field_order >> 1) + 1, .big);
