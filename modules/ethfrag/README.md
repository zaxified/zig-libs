...
accept/drop behavior against fragments built by hand. The committed data
`kernel_oracle.zig`'s `captures` were frozen from — and `modules/ethfrag/tools/capture.py`,
which ships alongside them — were generated by our own tooling: `capture.py`
builds real IPv4/IPv6 fragments by hand and captures this machine's own Linux
kernel's live reassembly verdict, run under `unshare --user --map-root-user
--net` (`d57f9ee0`); no third-party binary, corpus or upstream source is
involved.
