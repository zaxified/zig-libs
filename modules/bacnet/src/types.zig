...
pub const ErrorCode = enum(u16) {
    ...
    // ── Annex AB (BACnet/SC) ────────────────────────────────────────────────
    // The same `BACnetErrorCode` space, continued: these are the codes a
    // BACnet/SC `BVLC-Result` NAK carries (see `sc.ResultError`). The gap at
    // 87..142 is standard codes for services this module does not implement —
    // they still decode, as unnamed values, because the enum is non-exhaustive.
