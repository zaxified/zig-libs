...
This is a **Fable scaffold pass**: everything that *defines and measures*
anonymity is real; the irreducible *mixing* itself is a gated stub.
...
var mix = try loopix.Loopix.init(gpa, loopix.DEFAULT_CFG, seed);   // gated core
...
## What's real today vs. gated

- **Real (runs now):** the stratified topology + route selection (`routing.zig`,
  including a **real `sphinx` onion** round-trip that carries `(next_hop, delay)`
  per hop and peels end-to-end — the proof the Sphinx substrate works as a mix
  packet); the fixed-size mix-header codec (`types.zig`); the **entire anonymity
  measurement** (`adversary.zig` — effective anonymity set `2^H` +
...
- **Gated (`fable_core_implemented = false`):** the three irreducible mixing
  functions in `mixing.zig` — `sampleExpDelay` (memoryless integer delay),
  `scheduleRelease` (the Poisson mix's per-arrival hold), `nextCover` (the
  Poisson cover process). All `@panic("TODO(fable/core): ...")`. The one test
  that drives the real `Loopix` mix is gated and reports **SKIP** until the
  flag flips.
...
- `sampleExpDelay` / `scheduleRelease` / `nextCover` — the gated core.
  `Prng` / `CoverEvent` — the seeded randomness they use.
...
**1 skip** (the gated real-mix anonymity test). Highlights:
...
- the **FIFO positive control** run end-to-end through netsim, its transcript
  ...
  has teeth before the mixing core exists (plus a seed-sweep version under
  fuzzed faults);
