...
All four are represented in `suite.Mode`; `dhkem.zig` scaffolds
...
- **`I2OSP`/label-buffer sizing.** `labeledExpand`'s bounded 512-byte
  ...
  input.
- **PSK handling is entirely the caller's responsibility.** This module
  never generates, stores, or rotates a PSK — `keySchedule`'s
  `KeyScheduleError.InconsistentPsk` only checks the mode/PSK-presence
  *shape* (RFC 9180's `VerifyPSKInputs`), not PSK quality/freshness/
  storage, mirroring `tlsresume`'s STEK key material never being
  generated by that module either.
...
8. ✅ **`AuthEncap`/`AuthDecap`** for both KEMs (§4.1's `dh ‖ dh2` fold,
   ...
   pkRm ‖ pkSm`) — verified via the dtls/tlsresume self-consistency
   pattern (round trip to the identical `shared_secret`, wrong `pkS`
   diverges), since no official Appendix A auth-mode vector is embedded.
...
10. **`psk`/`auth_psk` PSK-mode KAT — still open (deliberately):**
    `VerifyPSKInputs` enforcement itself is implemented and tested
    (item 3), and the PSK inputs flow into the schedule per §5.1, but no
    RFC 9180 Appendix A PSK-mode vector is embedded yet to KAT the
    psk/psk_id-bearing derivation byte-exact — fetch §A.x.2 "PSK Setup
    Information" from the RFC before relying on psk/auth_psk modes.
