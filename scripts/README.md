...
| `stamps-key.sh` | The host half of a stamp's lane key, sourced by `test.sh` and `check-apps.sh`: the hash of what `native` resolves to (`zig targets`: CPU model and features, kernel, glibc), or only the kernel and glibc part when the lane pins `-Dcpu`, plus the CPU model name for the log. |
| `ci-cache-decide.sh` | Whether a CI lane's `.zig-cache` is saved as a new cache entry: only when this run added an output to `o/`, and not past a size cap (`ZIGLIBS_CACHE_CAP_MB`, default 4096). Before 2026-09-18 every commit re-saved every lane's tree, whether it had compiled anything or not, and the copies took the repository past its 10 GB quota. |
| `ci-cache-prune.sh` | Deletes the GitHub cache entries a newer entry of the same key family has superseded (family = key minus week, source hash and run id). Run by CI's `cache-prune` job, the only job with `actions: write`; `--dry-run` lists instead of deleting. |
...
| Before cutting a tag | `scripts/test.sh interop` | Re-takes every `modules/*/tools/interop.zig` anchor against its REAL peer — see below. Needs `scripts/lib/ci-environment.sh interop`; ~35 s warm |
...
- **Stamp** — `<module> TAB <lane> TAB <fingerprint> TAB <time>` in
  ...
  push lane uses `x86_64_v3+aes+pclmul`, because the runner pool mixes CPU models -- only
...
It is a triage aid, not a gate. Extraction is regex-based, so a large share of
a full-repo run is MISMATCH — ordinary prose sitting near a standards token,
not wrong citations. Read the `file:line` before believing one. UNFETCHABLE is
never a pass: those claims are simply unchecked.

### ⛔ NOT WIRED INTO ANY LANE, and that is decided (2026-09-09)

The question was raised by the A1 audit as R13 and answered by the owner: this
is an AUDIT instrument, not a test. A citation does not rot between two commits
the way code does — it rots over the years, and the moment to re-derive them is
when someone is auditing the module, not on every push.

Two things follow, and both are the reason to write this down rather than leave
the next reader to re-open it:

  - **It would need the network in a lane.** On 2026-09-09 this repository spent
    a session taking exactly that dependency OUT: `dns` had seven tests against
    real resolvers whose skip narration turned a slow lookup into a FAIL across
    217 modules (`modules/dns/tools/live.zig` is where they went). Adding a
    checker that fetches 143 documents would put it straight back.
  - **The proposal that came with the finding was sound and is still on the
    table if this is ever revisited**: freeze today's verdicts into a
    `citations-expected.tsv` and fail only on a NEW mismatch, the shape
    `portable-known-failures.tsv` already uses. It was not rejected on merit;
    it was not needed.

### Two extractor fixes, measured (2026-09-09)

Both are about the same failure: `QUOTE` pairs `"` characters sequentially
across a joined block, so wherever quotes mark LITERAL TOKENS rather than
quotation, the closing quote of one pairs with the opening quote of the next
and the text between them is reported as a citation.

  - **Fenced code was read as prose.** `blocks()` treated a ``` line as a block
    SEPARATOR rather than a toggle, so every fenced example became an ordinary
    paragraph — in `.md` and equally in `.zig`, whose doc comments fence their
    examples the same way. Cost: 35 claims, 28 of them MISMATCH.
  - **A claim that begins or ends mid-syntax is not a quotation.** No standard's
    sentence starts with `)` or `,` or ends with `(`. Cost: 104 claims, 97 of
    them MISMATCH.

⭐ Together they took MISMATCH from **499 to 374** and VERIFIED from 400 to 396.
The five lost VERIFIED are listed in the commit and every one is the same
artifact — a fragment of ABNF or pseudocode lying between two quoted literals,
which passed only because the whole block really is in the RFC. A coincidental
pass is precisely what VERIFIED cannot distinguish from a real one, so those
were never checks.

### ⛔ UNCHECKED, and why it used to be a PASS (fixed 2026-09-09)

A quote is split on ellipses and `[...]` editorial insertions, and only
fragments of four words or more are searched for — shorter runs match almost
anything. When a quote is short, or is mostly ellipsis, nothing survives that
filter, and `verify()` then returned **VERIFIED having compared zero characters
against the document**. A pass by construction, in the one tool built because an
unfounded claim is worse than a missing one.

Measured A/B on the same tree the day it was fixed: **410 VERIFIED before, 400
after, with 10 UNCHECKED**. Only MISMATCH sets a non-zero exit status; UNCHECKED
is a gap in coverage rather than a defect in the tree, and naming it is the
entire point.

⭐ What the ten turned out to be is worth knowing, because it says more about the
EXTRACTOR than about the citations: seven are not citations at all but code and
prose the regex mis-picked (`` ` prefix), NOT std's ``,
`` _ = try whois.arinIpQuery(&qbuf, ``, `` : the incomplete group ``). Three are
genuine quotes that are mostly ellipsis and cannot be checked as written
(`kex: ... cipher: ... MAC: ...`, `interpret ... as little-endian ... mod q`).
That ratio is the same precision problem behind the MISMATCH count, and it is
why tightening extraction is the step AFTER this one, not before it.

**Whole-repo run, 2026-09-09: 956 citations — 400 VERIFIED, 499 MISMATCH, 47
UNFETCHABLE, 10 UNCHECKED.** (2026-09-06, before UNCHECKED existed: 933
citations, 404 VERIFIED, 487 MISMATCH, 42 UNFETCHABLE, 4.3 s warm.) Before the same day it reported **12189** of them in
