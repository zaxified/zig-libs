...
| `check-fuzz-reach.py` | Refuses a fuzz harness that does not READ the input it is given. `zig build check-fuzz` proves a harness EXISTS; `std.testing.Smith`'s ranged draws (`valueRange*`, `valueWeighted`, `index`, `bool*`, and `value` of a type narrower than 64 bits) read eight bytes as a little-endian u64 and return the range MINIMUM unless that u64 already lies inside the range. So a target whose first draw is ranged replays every corpus seed — and every crash `--fuzz` minimises into a seed — as one fixed input. Two rules: **R1** the first draw must be faithful (`bytes*`/`slice*`, `value(T)` with T at least 64 bits, `[N]u8`); **R2** a buffer filled by a faithful draw must not then be sliced by a length that came from a ranged one — `smith.bytes(&buf); const len = smith.valueRangeAtMost(u16, 0, buf.len); decode(buf[0..len])` always decodes `""`, because `bytes` consumed the whole input. Measured at landing: **474 targets in 168 modules, 58 reaching their input, 416 collapsing across 152 modules** (105 by R1, 330 by R2 — one copy-pasted idiom). The fix is worked out and measured inside `netaddr`'s own harness comment: `smith.slice` takes it from 0 non-empty inputs to 9 of 12, 2 of them parsing. Runs `--advisory` until the burn-down reaches zero — advisory suppresses reach failures only, never a malformed or stale exemption. `--list` is the worklist grouped by module, `--module <name>` narrows it. A module states an exemption in its own SPEC.md/README.md as `**Fuzz-reach exemption:** STRUCTURED via fuzzA, fuzzB`, the same shape `check-fuzz` reads. ⛔ Blind to whether the harness ASSERTS anything about what it decoded. Runs on every gate. |
