...
| Module | What it does | Platform | Deps |
|---|---|---|---|
| [`entropy`](modules/entropy/README.md) *(crypto)* | Fail-closed entropy source — `fill` draws from `std.Io.randomSecure` or aborts the process; no generator, no silent degrade. **Panics on failure.** | any | — |
| [`netaddr`](modules/netaddr/README.md) *(net)* | IP parse/format (RFC 5952) + RFC 6724 source/dest selection + CIDR/Prefix ops (contains/overlaps/supernet, range↔prefix) | any | — |
...
| [`zstd`](modules/zstd/README.md) *(format)* | Zstandard (RFC 8878) compressor, levels 1-21 and negative levels — byte-identical to libzstd 1.5.7 `ZSTD_compress2`; decode with `std.compress.zstd` | any | — |
...
| [`opcua`](modules/opcua/README.md) | OPC-UA (IEC 62541) **client and server** — opc.tcp transport, secure channel (`#None` or Basic256Sha256 at Sign/SignAndEncrypt, both client and server), sessions, Read/Write/Browse/Call + subscriptions | any | rsa, x509 |
...
| [`wireguard`](modules/wireguard/README.md) | Native WireGuard device config over genetlink (retires `wg` shell-outs), plus the Noise_IKpsk2 handshake **and** the transport-data seal/open crypto data plane | **linux** | netlink, genetlink, chachapoly, entropy, netaddr |
...
| [`zstd`](modules/zstd/README.md) | Zstandard (RFC 8878) compressor, levels 1-21 and negative levels — byte-identical to libzstd 1.5.7 `ZSTD_compress2`; decode with `std.compress.zstd` | any | — |
...
| Module | What it does | Platform | Deps |
|---|---|---|---|
| [`framing`](modules/framing/README.md) *(format)* | Length-prefixed stream framing (`writeFrame`/`readFrame`) plus a generic JSON tagged-union envelope codec. | any | — |
...
| xz compression; zstd level 22 | bind `liblzma` / `libzstd` (`allyourcodebase/zstd` packages it for the Zig build system); or compress outside the process (the `xz`/`zstd` CLI in the consumer's pipeline) | Decoding is covered: std 0.16 ships `std.compress.xz` and `std.compress.zstd` decoders, and levels 1–21 of the zstd encoder are a module (`zstd`). What remains is an LZMA2 encoder — a large arc with no in-process consumer yet; today's consumers write plain bytes and compress in a backup/build step — and zstd's level 22, whose long-distance matching (on in libzstd above 64 MB inputs) is not ported. The one pure-Zig zstd encoder found (`muhammad-fiaz/zstd.zig` `fc6f968`, surveyed 2026-09-22) emitted frames the reference `zstd -d` rejects as corrupt, and ignored the level <!-- non-goal-ok: zstd --> |
