...
**Status:** 49 modules · 1412 tests (Zig 0.16, green in Debug + ReleaseFast) · **MIT** (see `LICENSE`;
...
| `snmp` | SNMP v1/v2c — BER/ASN.1 codec + OID + all 8 PDUs + manager client (get/next/bulk/set/walk) + a **trap/notification receiver** (v1 Trap / v2c Trap / Inform → one normalized `TrapEvent` + `Dispatcher` + `ackInform` byte-faithful Response ack) + **SNMPv3** (RFC 3412 message framing + ScopedPDU, plaintext/noAuthNoPriv) + **USM** `UsmSecurityParameters` (de)serializer (RFC 3414; auth/privacy crypto in progress), transport-agnostic seam | any | — |
