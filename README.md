...
**Status:** 115 modules · 3400 tests (Zig 0.16, green in Debug + ReleaseFast) · **MIT** (see `LICENSE`;
...
| `falcon` | Falcon-512 **and Falcon-1024** (FN-DSA, the NIST PQ lattice signature) — **verification + signing + all key/signature codecs**, NIST-Round-3-KAT-verified for both parameter sets (degree-generic negacyclic NTT mod 12289, SHAKE256 hash-to-point, compressed-sig codec, sk→pk consistency; the signing core — strict-float FFT + ffSampling + samplerZ — is byte-exact vs the NIST KATs for both sets); keygen (NTRUGen/NTRUSolve) still reserved | any | — |
