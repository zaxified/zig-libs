...
**Status:** 43 modules · 1175 tests (Zig 0.16, green in Debug + ReleaseFast) · **MIT** (see `LICENSE`;
...
| `mcp-http` | MCP **Streamable HTTP** transport (2025-06-18) — `POST /mcp` → JSON-RPC response / 202, as a `router` middleware over a `mcp.Server` (stateless; SSE stream + sessions are follow-ups) | any | router, http, mcp |
