...
**Status:** 34 modules · 931 tests (Zig 0.16, green in Debug + ReleaseFast) · **MIT** (see `LICENSE`;
...
|---|---|---|
| `http` | HTTP/1.1 client (TLS via `std.crypto.tls`) **and** server, hardened for direct exposure (peer addr, conn caps, size limits, slowloris timeouts, gzip). Also speaks **HTTP/2** — HPACK (RFC 7541) + framing/flow-control (RFC 9113) + an opt-in **h2c** server (same handler as h1). Not `std.http`. | netaddr |
