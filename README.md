...
**Status:** 49 modules · 1394 tests (Zig 0.16, green in Debug + ReleaseFast) · **MIT** (see `LICENSE`;
...
| `snmp` | SNMP v1/v2c — BER/ASN.1 codec + OID + all 8 PDUs + manager client (get/next/bulk/set/walk) + a **trap/notification receiver** (v1 Trap / v2c Trap / Inform → one normalized `TrapEvent` + `Dispatcher`), transport-agnostic seam | any | — |
